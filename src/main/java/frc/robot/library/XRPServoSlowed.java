...
public class XRPServoSlowed {
  ...
  /** XRPServo. */
  public XRPServoSlowed(int deviceNum) {
    ...
    if (xrpServoSimDevice != null) {
      ...
      m_simPosition = xrpServoSimDevice.createDouble("position", Direction.kOutput, 0);
    } else {
      m_simPosition = null;
    }
  }
   ...
   */
  public void setAngle(double angle, double current, double seconds) {
    double pos = current;
    if (angle < 0.0) {
      angle = 0.0;
    }
    ...
    double difference = angle - current; //(90)
    while (endTime-currentTime > 0) {
      double scaler = (Timer.getFPGATimestamp() - currentTime) / endTime - currentTime;
      pos = (current + (scaler * difference)) / 180;
    }
